...
基于间隔重复 (SM-2) 算法的自适应 CET-4 词汇学习系统。Docker 部署，**仅支持局域网访问**。
...
| `/learn` | 学习闯关（28 种学习模式：全随机 / 按首字母乱序 / 仅学某字母开头 → 复习 → 新词 → 总测 → 结算） |
| `/progress` | 轮次进度、熟练度分布 |
| `/settings` | 学习参数、时间估算 |
| `/admin` | 用户管理（需管理员权限） |
...
docker exec cet4-web node /app/dist/scripts/seed.js                  # 完整 4517 词
docker exec -e DB_PATH=/app/data/cet4_test.db cet4-web node /app/dist/scripts/seed_test.js  # 50 词
```
...
# API 测试（需挂载测试文件，容器内运行）
docker run --rm \
  -v "$(pwd)/tests:/app/tests" \
  -v "$(pwd)/vitest.config.ts:/app/vitest.config.ts" \
  -v "$(pwd)/src/server:/app/src/server" \
  -v "$(pwd)/tsconfig.json:/app/tsconfig.json" \
  cet4-web:latest npx vitest run                    # 全部 117 项
docker run --rm -v "$(pwd)/tests:/app/tests" -v "$(pwd)/vitest.config.ts:/app/vitest.config.ts" -v "$(pwd)/src/server:/app/src/server" -v "$(pwd)/tsconfig.json:/app/tsconfig.json" cet4-web:latest npx vitest run tests/server/routes/auth.test.ts  # 单文件
```
...
| 模块 | 测试文件 | 用例数 |
|------|---------|--------|
| 认证 | `tests/server/routes/auth.test.ts` | 21 |
| 学习 | `tests/server/routes/learn.test.ts` | 38 |
| 进度 | `tests/server/routes/progress.test.ts` | 12 |
| 管理 | `tests/server/routes/admin.test.ts` | 25 |
| 公开 | `tests/server/routes/public.test.ts` | 9 |
...
1. **预防** — `start.sh` 在 `docker compose up` 前 `touch ./data/cet4.db`，由宿主用户预创建，避免容器 root 进程创建后归属 root
2. **检测** — `docker compose up` 后检查 `data/cet4.db` 所有权，若仍为 root 则报错退出，提示执行 `fix-ownership`
3. **手动修复** — `bash start.sh fix-ownership` 随时可运行

## 生产部署

所有 API 端点使用真实数据库和生产级逻辑：
- SM-2 间隔重复算法驱动复习调度
- bcrypt 密码哈希存储
- JWT 认证（7 天有效期），含冻结用户检查
- 完整管理员权限体系（设置/冻结/删除用户）
- 自适应学习流程：复习 → 新词（全随机/按首字母乱序/仅学某字母） → 总测 → 轮次结算
- 按首字母范围学习（如"仅学 E 开头"），支持 A-Z 单选
- 117 项 API 集成测试覆盖全部 22 个端点
