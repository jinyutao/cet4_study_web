...
echo "  bash start.sh stop                              停止服务（数据库安全关闭）"
...
echo "  4. 停止服务:   bash start.sh stop"
echo "  5. 管理用户:   bash start.sh reset-password zhangsan"
...
if [ "$MODE" = "--test" ] || [ "$MODE" = "-t" ] || [ "$MODE" = "test" ]; then
  ...
  # 容器 root 进程在 volume 上创建的文件会归属 root，start.sh 已预创建 data/cet4_test.db
...
if [ "$MODE" = "stop" ]; then
  ...
  # checkpoint WAL，确保 SQLite 数据完整
  if docker ps --format '{{.Names}}' | grep -q '^cet4-web$'; then
    echo "执行数据库 WAL checkpoint..."
    docker exec cet4-web node -e "
      const Database = require('better-sqlite3');
      ['/app/data/cet4.db', '/app/data/cet4_test.db'].forEach(p => {
        try {
